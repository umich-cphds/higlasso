...
main and interaction effects in the presence of group structures
...
interaction detection. That being said, HiGLASSO is an time intensive algorithm
and hence there is a limit to how many variables can used at once. `higlasso` is
suitable to use on medium sized environmental health datasets (n = 1000, p = 10, 20).
...
features than the CRAN version doesn't have, so it may be a good idea to take
a look.

## Example
`higlasso` comes with an example data.frame, `higlasso.df` that is based off
the first simulation case in the paper. It contains a numeric response and
10 covariates, X1 - X10.
